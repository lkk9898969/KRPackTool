...
{
    private static readonly byte[] _mem_F4CAD0 =
    {
    ...
    private static readonly byte[] _mem_F4CEE0 =
    {
    ...
    private static readonly byte[] _mem_F4A300 =
    {
    ...
    public static unsafe byte[] GetPackedFileKey(byte[] fileChksum, int u1, string FileName)
    {
        ...
        var fileNameData = Encoding.GetEncoding("UTF-16").GetBytes(FileName);
        var temp_ebx = (uint)u1;
        var arr1 = new[] { 0d, 4294967296d };
        var temp1 = arr1[(uint)u1 >> 31];
        var dv = Sse2.LoadVector128(&u1);
        var dr = Sse2.ConvertToVector128Double(dv);
        var temp2 = Sse2.LoadVector128(&temp1);
        dr = Sse2.AddScalar(dr, temp2);
        var a1 = func_E321D0(dr);
        var temp_esi = Sse2.ConvertToInt32WithTruncation(a1);
        var arr2 = new int[temp_esi + 1];
        var i = temp_esi;
        while (temp_ebx != 0)
        {
            var a = 0xCCCCCCCDul * temp_ebx;
            var b = (uint)(a >> 32 >> 3); //edx
        ...
        for (i = 0; i < output.Length; i++)
        {
            ...
            var c = (int)(((sbyte)arr2[(i + 2) % arr2.Length] + i) & 0x8000000F); //esi at 4BA09F
            if (c < 0)
                c = ((c - 1) | -10) + 1;
            var outByte = (b + i) % 5; //ecx at 4BA0BC
    ...
    private static unsafe Vector128<double> func_E321D0(Vector128<double> _inxmm0)
    {
        ...
        while (true)
        {
            ...
            var _temp_F4A320 = 0x000FFFFFFFFFFFFF;
            var _temp_F4A330 = 0x3FDBC00000000000;
            var _temp_F4A340 = 0x428FFFFFFFFFF810;
            var _temp_F4A350 = 0x7FFFFFFF80000000;
            var _temp_F4A390 = 0x3FF0000000000000;
            var xmm1 = LoadVector128LH((double*)&_temp_F4A320);
            var xmm2 = LoadVector128LH((double*)&_temp_F4A330);
            var xmm3 = LoadVector128LH((double*)&_temp_F4A390);
            var xmm4 = LoadVector128LH((double*)&_temp_F4A340);
            var xmm6 = LoadVector128LH((double*)&_temp_F4A350);
            ...
            var xmm7 = LoadVector128FromByteArray(_mem_F4CAD0, temp_eax);
    ...
    {
        var output = Sse2.LoadVector128(address).AsDouble();
    ...
    private static unsafe Vector128<double> LoadVector128FromByteArray(byte[] arr, int index)
    {
        ...
        {
            var output = Sse2.LoadVector128(address + index).AsDouble();
            return output;
        }
    }
    ...
    {
        var output = unchecked((int)0x811C9DC5);
        foreach (var cha in anotherData)
        {
            var temp = cha ^ output;
