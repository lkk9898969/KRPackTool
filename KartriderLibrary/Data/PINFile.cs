...
public class PINFile
{
    ...
    {
        if (!File.Exists(path)) throw new Exception("Unable to locate PIN file.");
    ...
    {
        var inPacket = new InPacket(input);
        var inputBytes = inPacket.ReadBytes(inPacket.ReadInt());
    ...
    private static byte[] Encrypt(PINFile pinFile)
    {
        ...
        var outPacket2 = new OutPacket();
        var array = KREncodedBlock.Encode(outPacket.ToArray(), (KREncodedBlock.EncodeFlag)3, 862557747u);
    ...
    {
        if (p.ReadUInt() != 284099454) throw new InvalidDataException("This is not a valid PIN file.");
        ...
        AuthMethods = new List<AuthMethod>();
        for (var num = p.ReadInt(); num > 0; num--)
        {
            ...
            for (var num2 = p.ReadInt(); num2 > 0; num2--)
            {
        ...
        while (p.Available > 0)
        {
            var bmlObject = ReadBML(p);
            if (bmlObject != null) BmlObjects.Add(bmlObject);
        }
    }
    ...
    private void Write(OutPacket p)
    {
        ...
        foreach (var authMethod in AuthMethods)
        {
            ...
            foreach (var loginServer in authMethod.LoginServers) loginServer.Write(p);

            WriteBML(p, authMethod.ExtraConfig);
        }

        p.WriteBytes(new byte[NullsBeforeBmlObjects]);
        foreach (var bmlObject in BmlObjects) WriteBML(p, bmlObject);
    }
